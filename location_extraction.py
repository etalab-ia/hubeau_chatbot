...
def stem(word):
    ...
    return word[:-4] + re.sub(
        r"iens|ains|ards|ain|ien|ard|ois|oi|ens|en|ais|ai|ins|in|s$", "", word_, count=1
    )
...
def equal(x, y):
    ...
    return unicodedata.normalize("NFD", x.lower()).encode("ascii", "ignore").decode(
        "utf-8"
    ) == unicodedata.normalize("NFD", y.lower()).encode("ascii", "ignore").decode(
        "utf-8"
    )
...
def get_dependencies(query, word_list, nlp):
    ...
    return [
        ...
        if (v["head id"] in words and v["deprel"] in ["nmod", "amod"])
        or (v["deprel"] == "conj" and dependent[v["head id"]]["head id"] in words)
    ]
...
def get_locations_api(query, nlp, exact_match=True):
    ...
    locations = []
    for noun in nouns:
        url = "https://geo.api.gouv.fr/communes?nom={c}&fields=nom&format=json&geometry=centre".format(
            c=noun
        )
...
def get_location_demonym_dict(query, stemmed_dict, nlp):
    ...
    for demonym in demonyms:
        ...
        if (
            stemmed in stemmed_dict["communes"]
            or stemmed in stemmed_dict["departements"]
            or stemmed in stemmed_dict["regions"]
        ):
            locations[demonym] = list(
                set(
                    stemmed_dict["communes"].get(stemmed, [])
                    + stemmed_dict["departements"].get(stemmed, [])
                    + stemmed_dict["regions"].get(stemmed, [])
                )
            )
    return locations
...
def get_insee_commune(commune, parameter):
    ...
    """
    url = (
        "https://geo.api.gouv.fr/communes?" + parameter + "={c}&fields=nom,code,"
        "codeDepartement,codeRegion&format=json&geometry=centre".format(c=commune)
    )
    ...
    if parameter == "nom":
        exact_match = {
            code.get("code", ""): {
                "nom": code.get("nom", ""),
                "codeDepartement": code.get("codeDepartement", ""),
                "codeRegion": code["codeRegion"],
            }
        ...
        similar = {
            code.get("code", ""): {
                "nom": code.get("nom", ""),
                "codeDepartement": code.get("codeDepartement", ""),
                "codeRegion": code.get("codeRegion", ""),
            }
            for code in response
            if not equal(code.get("nom", ""), commune)
        }
...
def get_insee_departement(departement, parameter):
    ...
    url = (
        ...
        + parameter
        + "={c}&fields=nom,code,codeRegion"
        "&format=json&geometry=centre".format(c=departement)
    )
    ...
    if parameter == "nom":
        exact_match = {
            ...
            for code in response
            if equal(code.get("nom", ""), departement)
        }
        similar = {
            code.get("code", ""): {
                "nom": code.get("nom", ""),
            ...
            for code in response
            if not equal(code.get("nom", ""), departement)
        }
        return exact_match, similar
    elif parameter == "code":
        exact_match = {
...
def get_insee_region(region, parameter):
    ...
    """
    url = (
        "https://geo.api.gouv.fr/regions?" + parameter + "={c}&fields=nom,code"
        "&format=json&geometry=centre".format(c=region)
    )
    response = json.loads(requests.get(url).text)
    ...
    if parameter == "nom":
        ...
        exact_match = {
            code.get("code", ""): {"nom": code.get("nom", "")}
            for code in response
            if equal(code.get("nom", ""), region)
        }
        for region in exact_match:
            ...
            exact_match[region]["codesDepartements"] = [
                code.get("code", "") for code in deps
            ]

        similar = {
            code["code"]: {"nom": code["nom"]}
            for code in response
            if not equal(code["nom"], region)
        }
        for similar_region in similar:
            url = "https://geo.api.gouv.fr/regions/{c}/departements".format(
                c=similar_region
            )
            deps = json.loads(requests.get(url).text)
            similar[similar_region]["codesDepartements"] = [
                code.get("code", "") for code in deps
            ]
        ...
        exact_match = {
            code.get("code", ""): {"nom": code.get("nom", "")} for code in response
        }
        for region in exact_match:
            url = "https://geo.api.gouv.fr/regions/{c}/departements".format(c=region)
            deps = json.loads(requests.get(url).text)
...
def get_insee(locations):
    ...
    for location in locations:
        ...
        (
            exact_match[location]["communes"],
            similar[location]["communes"],
        ) = get_insee_commune(location_, "nom")
        (
            exact_match[location]["departements"],
            similar[location]["departements"],
        ) = get_insee_departement(location_, "nom")
        (
            exact_match[location]["regions"],
            similar[location]["regions"],
        ) = get_insee_region(location_, "nom")

        exact_match[location]["count"] = sum(
            [len(v) for k, v in exact_match[location].items()]
        )
...
def classify_nameNdemonym(query, locations, exact_match_, similar_, nlp):
    ...
    for loc in locations:
        insee_locations = (
            exact_match_[loc] if exact_match_[loc]["count"] > 0 else similar_[loc]
        )
        if insee_locations["count"] > 0:
            if (
                len(insee_locations["regions"]) == 0
                and len(insee_locations["departements"]) == 0
            ):  # The location is a commune
                communes[loc] = insee_locations["communes"]

            elif (
                len(insee_locations["regions"]) == 0
                and len(insee_locations["communes"]) == 0
            ):  # The location is a departement
                departements[loc] = insee_locations["departements"]
                code_departements = code_departements.union(
                    list(insee_locations["departements"].keys())
                )
            elif (
                len(insee_locations["departements"]) == 0
                and len(insee_locations["communes"]) == 0
            ):  # The location is a region
                regions[loc] = insee_locations["regions"]
                code_regions = code_regions.union(
                    list(insee_locations["regions"].keys())
                )

            elif (
                len(insee_locations["departements"]) > 0
                and len(insee_locations["communes"]) > 0
                and len(insee_locations["regions"]) == 0
            ):  # There's a departement and a commune that both have this name
                d_match = re.search(
                    "(departement|département|departements|départements)", query
                )
                if d_match:
                    d_dependencies = (
                        get_dependencies(
                    ...
                    if loc in d_dependencies:
                        ...
                        dep_regions, com_regions = [
                            d["codeRegion"]
                            for d in insee_locations["departements"].values()
                        ], [
                            d["codeRegion"]
                            for d in insee_locations["communes"].values()
                        ]
                    ...
                    dep_regions, com_regions = [
                        d["codeRegion"]
                        for d in insee_locations["departements"].values()
                    ], [d["codeRegion"] for d in insee_locations["communes"].values()]
                    inter = list(set(dep_regions).intersection(com_regions))
                    if len(inter) == 0:
                        com_departements = [
                            d["codeDepartement"]
                ...
                if r_match:
                    r_dependencies = (
                        get_dependencies(
                            query, ["region", "région", "regions", "régions"], nlp
                        )
                        if r_dependencies is None
                        else r_dependencies
                    )
    ...
    if len(regions) == 0:
        ...
        for loc, values in check_again.items():
            insee_locations = exact_match_[loc]
            if (
                len(code_regions.intersection(values["com_regions"])) == 0
                and len(code_regions.intersection(values["dep_regions"])) == 0
            ):
                communes[loc] = insee_locations["communes"]
            if len(set(values["dep_regions"]).intersection(code_regions)) > 0:
                departements[loc] = insee_locations["departements"]
            if (
                len(set(values["com_regions"]).intersection(code_regions)) > 0
                or len(set(values["com_departements"]).intersection(code_departements))
                > 0
            ):
                communes[loc] = insee_locations["communes"]
        return communes, departements, regions
...
def classify_geoloc(query, locations):
    ...
    code_c, code_d, code_r = (
        response.get("code", ""),
        response.get("codeDepartement", ""),
        response.get("codeRegion", ""),
    )
...
def get_locations(
    query_, all_location_names, demonym_dict, model, nlp, ip_address=None
):
    """
...
def get_relevant(communes, departements, regions):
    ...
    if (
        len(regions) == 0 and len(departements) == 0 and len(communes) > 0
    ):  # Simple case where there are only communes
        for name, content in communes.items():
            coms_to_keep.update(content)

    elif (
        len(regions) == 0 and len(departements) > 0 and len(communes) == 0
    ):  # simple case where there are only departements
        for name, content in departements.items():
            deps_to_keep.update(content)

    elif (
        len(regions) > 0 and len(departements) == 0 and len(communes) == 0
    ):  # simple case where there are only regions
        ...
        for name, content in communes.items():
            if (
                len(content) == 1
            ):  # commune name is unique, commune is added its departement is ommited
                coms_to_keep.update(content)
        ...
        for name, content in communes.items():
            if (
                len(content) == 1
            ):  # commune name is unique, commune is added its region is ommited
                coms_to_keep.update(content)
        ...
        for name, content in departements.items():
            if (
                len(content) == 1
            ):  # dep name is unique, dep is added its region is ommited
                deps_to_keep.update(content)
                regs_to_omit.add(list(content.values())[0]["codeRegion"])
        ...
        for name, content in communes.items():
            if (
                ...
                for com, info in content.items():
                    if (
                        info["codeDepartement"] in dep_codes
                        or info["codeRegion"] in regs_codes
                    ):
                        _[com] = info
                        deps_to_omit.add(info["codeDepartement"])
                        regs_to_omit.add(info["codeRegion"])
