...
def insee_to_bss(code_location, type_location):
    ...
    if exists > 0:
        ...
        bss = {
            station["code_bss"]: {
                "date_debut_mesure": station["date_debut_mesure"],
                "date_fin_mesure": station["date_fin_mesure"],
            }
...
def get_mesure_piezo(station, start_date=None, end_date=None):
    ...
    """
    url = (
        f'{"https://hubeau.eaufrance.fr/api/v1/niveaux_nappes/chroniques?code_bss={bss}".format(bss=station)}'
...
def get_details_from_bss(bss):
    url = "https://hubeau.eaufrance.fr/api/v1/niveaux_nappes/stations?code_bss={c}&format=json&size=200".format(
        c=bss
    )
    exists = json.loads(requests.get(url).text)["count"]
    if exists > 0:
        ...
        return {
            info["code_bss"]: {
                "code_commune": info["code_commune_insee"],
    ...
    if info != -1:
        ...
        for station, _ in infos.items():
            ...
            if date is not None:
                ...
                if last_mesure_date >= datetime(
                    2005, 1, 1
                ):  # the last mesure date must be later than 01-01-2005
        ...
        if len(dates) > 0:
            for date in dates:
                ...
                for code, details in details_.items():
                    result = get_mesure_piezo(
                        code,
                        start_date=date.get("start_date", None),
                        end_date=date.get("end_date", None),
                    )
                    if details["code_commune"] not in data_:
                        data_[details["code_commune"]] = {
                            "data": {},
                            "name": details["nom_commune"],
                        }

                    data_[details["code_commune"]]["data"][code] = {
                        "date_debut_mesure": details["date_debut_mesure"],
...
def get_table_data_locations(coms_to_keep, deps_to_keep, regs_to_keep, dates_to_keep):
    ...
    if len(dates_to_keep) > 0:
        for date in dates_to_keep:
            data = {}
            for gran, gran_dict in {
                "commune": coms_to_keep,
                "departement": deps_to_keep,
                "region": regs_to_keep,
            }.items():
...
def show_results(
    query, flair_model, nlp, heideltime_parser, nb_mesures, all_locations, demonym_dict
):
    final_result = {}
    ...
    if len(bss_codes) > 0:
        ...
        coms_to_keep, deps_to_keep, regs_to_keep = get_relevant(
        ...
        data, stations = get_table_data_locations(
            coms_to_keep, deps_to_keep, regs_to_keep, dates_to_keep
        )
    ...
    for data_ in data:
        ...
        final_result["recaps"].append(
            {
                "start_date": data_.get("start_date", ""),
                "end_date": data_.get("end_date", ""),
                "recap": dict_general,
            }
        )
